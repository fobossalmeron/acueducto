...
const CookieMessage = React.memo(({ t, hasLoaded }) => {
    ...
    const checkScroll = useCallback(() => {
        ...
        if ((layoutWrapper && layoutWrapper.scrollTop > 100) ||
            window.scrollY > 100) {
            document.body.onscroll = null;
            if (layoutWrapper)
                layoutWrapper.onscroll = null;
            checkForConsent();
    ...
    useEffect(() => {
        if (hasLoaded && showConsentMessage) {
            delayForLoading(800).then(() => {
                ...
                const layoutWrapper = document.querySelector('#LayoutWrapper');
                if (layoutWrapper)
                    layoutWrapper.onscroll = checkScroll;
            });
        }
        return () => {
            document.body.onscroll = null;
    ...
    return (_jsx(Wrapper, { "$clickable": hasToConsent, children: _jsxs(Border, { children: [_jsxs(Divider, { onClick: consentToCookies, children: [_jsx(Button, { children: _jsx("span", { children: tt.title }) }), _jsx(CrossContainer, {})] }), _jsxs("p", { children: [tt.p, _jsx(Link, { href: "/cookies", passHref: true, legacyBehavior: true, children: _jsx(Hoverable, { children: tt.link }) }), tt.p_continued] })] }) }));
});
