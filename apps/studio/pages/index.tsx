...
import { useLocalizedContent, Button } from '@acueducto/shared';
        ...
        <Land id="land" $locale={locale}>
          <LandContainer>
            <Fade cascade delay={0.4}>
              <p className="h1">{t?.landing?.heading || 'Loading...'}</p>
              <h1 className="h2">{t?.landing?.tagline || 'Loading...'}</h1>
              <div className="bg-background border-card-border xs:gap-4 flex w-fit gap-2 rounded-full border p-3 md:p-4">
                ...
                >
                  <Button
                    text={t?.landing?.ctaContact || 'Contact'}
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const pt = ssrLocale({ locale, fileName: 'home.json' });
  return {
    props: {
      pt,
      locale,
